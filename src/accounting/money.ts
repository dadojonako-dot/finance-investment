...
export function dateValue(value: unknown): Date {
  ...
  if (calendar) {const check=new Date(calendar[1]+'T00:00:00Z');if(!Number.isFinite(check.getTime())||check.toISOString().slice(0,10)!==calendar[1])throw new InputError('Некорректная дата');}
